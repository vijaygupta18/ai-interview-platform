...
function getSTTConfig() {
  ...
  return { provider: "deepgram", wsUrl: `wss://api.deepgram.com/v1/listen?model=nova-2&language=${language}&punctuate=true&interim_results=true&endpointing=800&vad_events=true`, protocols: ["token", k] };
}
...
function addWSProxy(server) {
  ...
  wss.on("connection", (clientWs) => {
    ...
    clientWs.on("close", () => { clearInterval(ping); if (upstream.readyState <= 1) upstream.terminate(); });
