        ...
        if (hasStarted && interview.status !== "completed") {
          ...
          try {
            ...
            if (violationRes.ok) {
              ...
              const maxStrikes = parseInt(process.env.NEXT_PUBLIC_MAX_PROCTORING_STRIKES || "5");
        ...
        <div className="fixed left-1/2 -translate-x-1/2 top-16 z-50">
          <div className={`flex items-center gap-3 rounded-xl px-5 py-3 shadow-lg border ${
            ...
            <div className="flex gap-1">
              {Array.from({ length: Math.min(parseInt(process.env.NEXT_PUBLIC_MAX_PROCTORING_STRIKES || "5"), 10) }, (_, i) => i + 1).map((i) => (
            ...
            <span className={`text-sm font-medium ${proctoringWarnings >= 3 ? "text-red-200" : "text-yellow-200"}`}>
              Warning {proctoringWarnings}/{parseInt(process.env.NEXT_PUBLIC_MAX_PROCTORING_STRIKES || "5")} — {proctoringWarnings >= parseInt(process.env.NEXT_PUBLIC_MAX_PROCTORING_STRIKES || "5") - 1 ? "Next violation will end the interview" : "Please stay focused and look at the screen"}
            </span>
          </div>
        </div>
      )}
